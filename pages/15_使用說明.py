...
# ── 系統憲法(2026-09-11 立憲:三層架構,下單權只給一層)──
st.markdown("### 🏛️ 系統憲法(先讀這個)")
st.info("""**三層架構——任何頁面看到再心動,下單只能走 L0:**

**L0 執行層(唯一能下單)**:錯殺V轉(EV+8.88%)/鯨魚5日(+3.19%)/我的ETF月調(+7.7pp/月)。
進場必過:體檢卡(🔴≥2 減半或不進)+ 決策日誌(價/論點/停損)。

**L1 裁決層(只在這些時點做決定)**:晨報(看)→ ETF日檢 → 週六週檢視 → 每月10日記分卡。
盤中看儀表手癢=違憲。

**L2 儀表層(context,不喊單)**:其餘所有頁面——榜單/籌碼/權證/RRG/法說/雷達,
價值是讓 L1 的決定更好。
